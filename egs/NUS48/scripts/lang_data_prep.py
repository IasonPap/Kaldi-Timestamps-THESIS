...
def makeWordsFile (text_path, DstDir):
	"""
	Makes a txt file with all the unique words from "text" file. 
	"""
	with open(text_path, mode = "r") as reader:
		lines = reader.read().splitlines(keepends = False)
	words = []
	for line in lines:
		wordsInLine = line.split(" ")[1:]
		for word in wordsInLine:
			words.append(word)
	words = sorted(list(dict.fromkeys(words)))
	with open(os.path.join(DstDir, "words.txt"), mode="w+") as writer:
		for word in words:
			writer.write( word + "\n" )
...
def main():
	...
	# 					action="store_true")
	parser.add_argument("--words", help="calls makeWordsFile", action="store_true")
	...
	if args.words:
		text_file = os.path.join(args.DataDir, "text")
		if os.path.isfile(text_file):
			makeWordsFile(text_file, args.DstDir)
	if args.f1:
