...
def get_smaller_list(list_A, list_B):
	return list_A if len(list_A) < len(list_B) else list_B

def get_bigger_list(list_A, list_B):
	return listA if len(listA) > len(listB) else listB
...
def find_string(my_list, target):
	...
	# Firsts Sorts the List and the applies binary search to find the target string.
...
def check_list_elems_1_by_1(listA, listB):
	smaller_list = get_smaller_list(listA, listB)
	other_list = get_bigger_list(listA, listB)
